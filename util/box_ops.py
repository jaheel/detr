...
def box_cxcywh_to_xyxy(x):
    """ (cx, cy, w, h) to (x_left_up, y_left_up, x_right_down, y_right_down)
    ...
    x_c, y_c, w, h = x.unbind(-1) # 将第二维度4个数据分解成(x_c, y_c, w, h)
...
def box_xyxy_to_cxcywh(x):
    """(x_left_up, y_left_up, x_right_down, y_right_down) to (cx, cy, w, h)
...
def generalized_box_iou(boxes1, boxes2):
    ...
    Parameters
    ----------
    boxes1 : {float, matrix} of shape (batch_size * num_target_boxes, 4)

    boxes2 : {float, matrix} of shape (batch_size * num_target_boxes, 4)
    """
